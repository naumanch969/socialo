...
export const auth = async (req, res, next) => {
    try {
        if (!req.headers.auth_token) return res.status(400).json({ message: "invalid user. Please sign in to do something with memory" })
        ...
        //  now we can get userId by req.userId
        } else {
