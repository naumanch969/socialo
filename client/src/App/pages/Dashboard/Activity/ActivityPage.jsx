...
import { getUserActivities, createActivity, updateLink, deleteLink, updateHeading } from "../../../../redux/actions/activity"

const ActivityPage = () => {
    const {
        activityState, setActivityState, resetedActivityData, showActivityForm,
        userState,
        headingRef, linkRef,
        TLHeadingValue, TLLinkValue,
    ...
    // signUp Time
    let { registeredAt, loginAt } = userState.user
    // loginAt || registeredAt
    let signUpTimeObj = { year: registeredAt?.year, month: registeredAt?.month, day: registeredAt?.date, hour: registeredAt?.hour, minute: registeredAt?.minute, second: registeredAt?.second }
    ...
    useEffect(() => {
        setDiff(daysPassed + 1 - activities.length)
        let updatedArr = activities.map((act) => {
            return {
                ...act,
    ...
    return (
        <div onClick={handleClick} className="w-full h-full flex flex-col items-center justify-center  " >
            ...
            <div className={`w-[88%] mt-[2rem] pb-[2rem] flex flex-col items-center  `} >
